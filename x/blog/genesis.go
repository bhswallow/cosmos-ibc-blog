...
func InitGenesis(ctx sdk.Context, k keeper.Keeper, genState types.GenesisState) {
	...
	// Set all the updatePost
	for _, elem := range genState.UpdatePostList {
		k.SetUpdatePost(ctx, elem)
	}

	// Set updatePost count
	k.SetUpdatePostCount(ctx, genState.UpdatePostCount)
...
func ExportGenesis(ctx sdk.Context, k keeper.Keeper) *types.GenesisState {
	...
	genesis.UpdatePostList = k.GetAllUpdatePost(ctx)
	genesis.UpdatePostCount = k.GetUpdatePostCount(ctx)
