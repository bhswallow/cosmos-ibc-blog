...
func (gs GenesisState) Validate() error {
	...
	// Check for duplicated ID in updatePost
	updatePostIdMap := make(map[uint64]bool)
	updatePostCount := gs.GetUpdatePostCount()
	for _, elem := range gs.UpdatePostList {
		if _, ok := updatePostIdMap[elem.Id]; ok {
			return fmt.Errorf("duplicated id for updatePost")
		}
		if elem.Id >= updatePostCount {
			return fmt.Errorf("updatePost id should be lower or equal than the last id")
		}
		updatePostIdMap[elem.Id] = true
	}
